...
// Paleta de cores para cada linha (0 = topo, 7 = base)
const lineColors = [
  '#0080ff', // linha 0 (superior)
  '#00cc44', // linha 1
  '#b266ff', // linha 2
  '#ffc107', // linha 3
  '#17a2b8', // linha 4
...
Promise.all([
  ...
  const hilbertGen = d3.hilbert().order(hilbertOrderExp).canvasWidth(hilbertCanvas);
  ...
  function gridUpdate(frame) {
    gridCells
      ...
      .attr('stroke', d => {
        const cellFeats = feats.find(f => f.id === d.properties.id);
        const iy = cellFeats.iy;
        return lineColors[iy];
      })
      .attr('stroke-width', CELL_BORDER)
      .on('mouseover', function(event, d) {
        const cellFeats = feats.find(f => f.id == d.properties.id);
        showTooltip(
          `<strong>ID:</strong> ${d.properties.id}<br>
           <strong>Coord:</strong> (${cellFeats.ix},${cellFeats.iy})<br>
           <strong>Linha:</strong> ${cellFeats.iy + 1}`,
          event
        );
      ...
      .on('mouseout', function(event, d) {
        ...
        const iy = cellFeats.iy;
        d3.select(this)
          .attr('stroke', lineColors[iy])
          .attr('stroke-width', CELL_BORDER);
        hideTooltip();
      });
  }

  function getBarColorFromLine(id) {
    const cellFeats = feats.find(f => f.id === id);
    if (!cellFeats) return '#eee';
    return lineColors[cellFeats.iy];
  }
  ...
  function renderProjection(targetSvg, order, curveName, showFocusLine = true) {
    ...
    // Barras coloridas por linha
    gProj.append('g').attr('class', 'extremos-bar')
      .selectAll('rect')
      .data(order)
      .join('rect')
      .attr('x', 0)
      .attr('y', (id, i) => y(i + 1) - bandH)
      .attr('width', sideBarW)
      .attr('height', bandH)
      .attr('fill', id => getBarColorFromLine(id));
    const gHeat = gProj.append('g').attr('transform', `translate(${sideBarW + gapW},0)`);
    gHeat.selectAll('g.row')
      .data(order)
      .join('g')
      .attr('class', 'row')
      .attr('transform', (id, r) => `translate(0,${y(r + 1) - bandH})`)
      .each(function(id, r) {
          ...
          .on('mouseover', function(event, tk) {
            ...
            showTooltip(
              `<strong>Célula ID:</strong> ${id}<br>
               ...
               <strong>Linha:</strong> ${f.iy + 1}<br>
               <strong>Posição:</strong> ${curveName} ${r + 1}/${S}<br>
               <strong>Tempo:</strong> ${timeKeys.indexOf(tk)}<br>
               <strong>Valor:</strong> ${v.toFixed(2)}`,
              event
            );
  ...
  if (btnModeComparison) btnModeComparison.addEventListener('click', () => switchMode('comparison'));
  ...
  if (btnMorton) btnMorton.addEventListener('click', () => {
    currentCurve = 'morton';
  ...
  if (btnHilbert) btnHilbert.addEventListener('click', () => {
